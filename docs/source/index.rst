.. py3minifier documentation master file, created by
   sphinx-quickstart on Sat May 24 14:25:46 2014.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

py3minifier - Minify, obfuscate, and compress Python code
========================================================
...
.. code-block:: sh
    ...
    $ py3minifier --obfuscate /tmp/tumult.py
    #!/usr/bin/env python
    T=ImportError
    q=print
    m=False
...
That's all fine and good but py3minifier can go the extra mile and also
*compress* your code using gzip, bz2, or even lzma using a special container:

.. code-block:: sh

    $ py3minifier --obfuscate --gzip /tmp/tumult.py
    #!/usr/bin/env python3
    import zlib, base64
...
.. code-block:: sh
    ...
    $ py3minifier --obfuscate --pyz=/tmp/tumult.pyz /tmp/tumult.py
...
py3minifier will automatically find locally-imported modules and include them in
the container (more on this below).

To properly demonstrate the effectiveness of each minification, obfuscation,
and compression method we'll minify the py3minifier.py code itself.  Here's the
results:

    ================ =========   ===========
    Method           File Size   % Reduction
    ================ =========   ===========
    py3minifier.py    17987       0%
...
For the .pyz comparison we'll need to add up the total sum of py3minifier.py
...
but for all those files.  To do things like this py3minifier includes the
...
Py3minifier can also work on a whole directory of Python scripts:

.. code-block:: sh

    $ py3minifier --destdir=/tmp/minified_py3minifier py3minifier/*.py
    py3minifier/analyze.py (12259) reduced to 7009 bytes (57.17% of original size)
    py3minifier/compression.py (11293) reduced to 4880 bytes (43.21% of original size)
    py3minifier/__init__.py (284) reduced to 193 bytes (67.96% of original size)
    py3minifier/minification.py (18639) reduced to 8586 bytes (46.06% of original size)
    py3minifier/obfuscate.py (26474) reduced to 13582 bytes (51.3% of original size)
    py3minifier/py3minifier.py (17987) reduced to 8439 bytes (46.92% of original size)
    py3minifier/token_utils.py (1175) reduced to 604 bytes (51.4% of original size)
    Overall size reduction: 49.13% of original size
    $ du -hs /tmp/minified_py3minifier/
    64K     /tmp/minified_py3minifier/
...
.. code-block:: sh
    ...
    $ rm -rf /tmp/minified_py3minifier # Clean up after ourselves first
    $ py3minifier --destdir=/tmp/minified_py3minifier --gzip py3minifier/*.py
    py3minifier/analyze.py (12259) reduced to 2773 bytes (22.62% of original size)
    py3minifier/compression.py (11293) reduced to 2165 bytes (19.17% of original size)
    py3minifier/__init__.py (284) reduced to 289 bytes (101.76% of original size)
    py3minifier/minification.py (18639) reduced to 2829 bytes (15.18% of original size)
    py3minifier/obfuscate.py (26474) reduced to 3924 bytes (14.82% of original size)
    py3minifier/py3minifier.py (17987) reduced to 3652 bytes (20.3% of original size)
    py3minifier/token_utils.py (1175) reduced to 497 bytes (42.3% of original size)
...
.. code-block:: sh
    ...
    $ rm -rf /tmp/minified_py3minifier # Clean up after ourselves first
    $ py3minifier --destdir=/tmp/minified_py3minifier --bzip2 py3minifier/*.py
    py3minifier/analyze.py (12259) reduced to 2951 bytes (24.07% of original size)
    py3minifier/compression.py (11293) reduced to 2435 bytes (21.56% of original size)
    py3minifier/__init__.py (284) reduced to 327 bytes (115.14% of original size)
    py3minifier/minification.py (18639) reduced to 2995 bytes (16.07% of original size)
    py3minifier/obfuscate.py (26474) reduced to 3986 bytes (15.06% of original size)
    py3minifier/py3minifier.py (17987) reduced to 3926 bytes (21.83% of original size)
    py3minifier/token_utils.py (1175) reduced to 555 bytes (47.23% of original size)
...
.. code-block:: sh
    ...
    $ rm -rf /tmp/minified_py3minifier # Clean up after ourselves first
    $ py3minifier --destdir=/tmp/minified_py3minifier --lzma py3minifier/*.py
    py3minifier/analyze.py (12259) reduced to 2801 bytes (22.85% of original size)
    py3minifier/compression.py (11293) reduced to 2273 bytes (20.13% of original size)
    py3minifier/__init__.py (284) reduced to 361 bytes (127.11% of original size)
    py3minifier/minification.py (18639) reduced to 2881 bytes (15.46% of original size)
    py3minifier/obfuscate.py (26474) reduced to 3904 bytes (14.75% of original size)
    py3minifier/py3minifier.py (17987) reduced to 3720 bytes (20.68% of original size)
    py3minifier/token_utils.py (1175) reduced to 601 bytes (51.15% of original size)
...
.. code-block:: sh
    ...
    $ py3minifier --pyz=/tmp/py3minifier.pyz py3minifier.py
    py3minifier.py saved as compressed executable zip: /tmp/py3minifier.pyz
    The following modules were automatically included (as automagic dependencies):
...
py3minifier.pyz by re-minifying tumult.py...

.. note:: Remember, the more code there is the more space will be saved.

.. code-block:: sh

    $ /tmp/py3minifier.pyz /tmp/tumult.py
    #!/usr/bin/env python
    try:
     import demiurgic
    except ImportError:
    ...
    if __name__=="__main__":
     ...
     f=Foo("epicaricacy","perseverate")
     f.test("Codswallop")
    # Created by py3minifier.py (https://github.com/koritcal/py3minifier)
...
py3minifier to use name replacements of the given size.  So instead of trying to
minimize the amount of characters used for replacements let's make them HUGE:

.. code-block:: sh

    $ py3minifier --nonlatin --replacement-length=50 /tmp/tumult.py
    #!/usr/bin/env python3
    ...
    # Created by py3minifier (https://github.com/kortical/py3minifier)
